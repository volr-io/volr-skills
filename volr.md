...
Get these from your .env file or the Volr Dashboard.
...
- **No config files in project directory**: The CLI stores all configuration (auth, project) in `~/.volr/config.json`. There is no `volr.json` or similar file created in the user's project directory.
